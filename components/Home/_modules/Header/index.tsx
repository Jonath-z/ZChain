...
const Menu = ({ className, py,activeMenu }: IMenuClass) => {
  ...
  const [createNftActive, setCreateNftActive] = useState("text-white");
...
const Header = ({ background, onChange ,isSearchAvailabe, activeMenu}: IProps) => {
    ...
    <>
      {!isMobile &&
        ...
        {isSearchAvailabe && <Search onChange={onChange} />}
          <Menu className="text-white flex flex-row justify-around w-96" py='py-0' activeMenu={activeMenu}/>
        <p className={`bg-[#ffa503] py-3 px-10 rounded-md hover:bg-opacity-80 flex items-center`}>
          ...
          />
            {isMenu &&
              <div className="bg-black flex flex-col absolute right-0 px-10 py-5 top-24 rounded-l-md z-30">
                 {isSearchAvailabe && <Search onChange={onChange} />}
                <Menu className="flex flex-col text-white" py="py-3" activeMenu={activeMenu}/>
                <p className={`bg-[#ffa503] py-3 px-10 my-5 rounded-md hover:bg-opacity-80 flex items-center`}>
                  <MdAccountBalanceWallet />
                  <span className="px-2">
                    {accountBalance} ETH
                  </span>
